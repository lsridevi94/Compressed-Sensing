...
# Defining the weights_init function that takes as input a neural network m and 
#that will initialize all its weights.
def weights_init(m):
...
class G(nn.Module): # We introduce a class to define the generator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the generator.
        ...
        self.main = nn.Sequential( 
                # We create a meta module of a neural network that will contain a 
                #sequence of modules (convolutions, full connections, etc.).
    ...
    def forward(self, input): 
        # We define the forward function that takes as argument an 
        #input that will be fed to the neural network, and that will return 
        #the output containing the generated images.
        output = self.main(input) 
        # We forward propagate the signal through the whole neural network 
        #of the generator defined by self.main.
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        X=input.data.numpy()
        #X=tf.Session().run(tf.constant(input))
        ...
        Xat1=Xat.reshape((64,3,64,3))
        #Xat=np.array(lasso.coef_).T
        
        Xa=torch.from_numpy(Xat1)
        noise=Variable(Xa)
        ...
        output = netD(fake.detach())
            # We forward propagate the fake generated images into the 
            #neural network of the discriminator to get the prediction (a value between 0 and 1).
